...
Imagen de fondo:
- Eclipse solar obtenido de Pixabay (licencia de uso gratuito)
Autor: kesie91

Descripción:
- Imagen de fondo cargada con preload() + loadImage()
- Tipografía externa cargada con preload() + loadFont()
- Elementos animados:
  1) Sol central con halo (animación circular mediante elipses y alpha)
  2) Luna (círculo negro) cruza siempre con movimiento ondulatorio (sin())
  3) Corona solar (rayos circulares) aparecen en eclipses fuertes
  4) Atmósfera orgánica con partículas animadas mediante noise() (criterio extra)
  5) Número “drop” que cae desde el sol mostrando el máximo eclipse alcanzado en cada pasada
- Cálculo de eclipse” (0..1) a partir de dist() entre luna y sol + map() para traducirlo a efectos visuales:
  oscurecimiento global, intensidad del halo y aparición de la corona
- Interacción mínima: tecla N alterna modo texto y no texto
*/
...
function drawCorona(cover) {
  // Bajamos umbral para que se aprecie con más frecuencia
...
function resetMoonFromLeft() {
  // 15% total, 25% parcial, resto libre
  let r = random(0, 1);
...
function spawnDrop(maxCover) {
  // Si ya hay un número cayendo, no generamos otro
  if (dropActive) return;

  // eclipse pequeño no mostramos número
